...
概念・アーキテクチャの詳細は indexion wiki（[`.indexion/wiki/`](.indexion/wiki/index.md)）を正本とする。編集前に hub ページ（Architecture / Response Pipeline / Agent Ownership）を読むこと。

## プロジェクト概要

- **目的**: 生成AIを主エンジンにせず、ルールベースと乱数で「幼稚園児くらいのアホの子」っぽく振る舞う Discord bot
...
リポジトリ知識は `.indexion/wiki/` に集約している。Windows では `%USERPROFILE%\.indexion\bin` を PATH に入れるか、フルパスで `indexion.exe` を呼ぶ。

```powershell
# 入口
Get-Content .indexion/wiki/index.md

# 検索
indexion search "じゃんけん" .indexion/wiki/

# 編集前のオーナー推定
indexion agent orient --task "<英語またはコード語彙のタスク要約>" --output .indexion/cache/agent/orient.md .

# ソース変更で古くなった wiki ページ検知
...
wiki の `.md` を直接編集した場合は、必ず `wiki pages update` で manifest / 検索 index / log を同期する（直接 Edit だけだと index が腐る）。

主要ページ:

| ページ | 内容 |
|--------|------|
| [overview](.indexion/wiki/overview.md) | 目的・スタック |
| [architecture](.indexion/wiki/architecture.md) | メッセージ処理フロー |
| [intent-classification](.indexion/wiki/intent-classification.md) | `MessageIntent` |
| [response-pipeline](.indexion/wiki/response-pipeline.md) | `ResponsePlanner` |
| [memory-store](.indexion/wiki/memory-store.md) | Prisma / `MemoryStore` |
| [feature-engines](.indexion/wiki/feature-engines.md) | 占い・俳句・ダイス等 |
| [discord-integration](.indexion/wiki/discord-integration.md) | Gateway・スラッシュコマンド |
| [agent-ownership](.indexion/wiki/agent-ownership.md) | 編集先マップ |
...
src/index.ts          … Client 初期化、MessageCreate / InteractionCreate
    ↓
src/message-guard.ts  … 対象外・重複メッセージの除外
    ↓
...
src/*.ts              … 機能エンジン（占い・俳句・じゃんけん・ダイス・気分など）
```

詳細は wiki の [Architecture](.indexion/wiki/architecture.md)。
...
| 占い・俳句・じゃんけん・ダイス・クイズ・Wiki | 各 `src/*.ts` エンジン |
| デフォルトの語彙・リアクション | `src/default-brain.ts` |
| 気分・なつき・雑談口調 | `mood.ts` / `affection.ts` / `personality.ts` / `chatter-engine.ts` |
| 静音・チャンネル活動・要約 | `quiet-channel.ts` / `channel-activity.ts` / `channel-summary.ts` |
...
- **分類・純粋関数は必ずテストを追加**（`classifier`, `confusion`, `haiku`, `janken`, `dice` など）
...
8. アーキテクチャやオーナーシップが変わったら `.indexion/wiki/` を `wiki pages update` し、`wiki lint` がクリーンであることを確認する

## 参考

- 人間向けセットアップ手順: [README.md](README.md)
- プロジェクト wiki: [.indexion/wiki/index.md](.indexion/wiki/index.md)
