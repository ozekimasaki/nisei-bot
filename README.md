...
- `/nisei_ai_status provider:openai|google|claude|xai` … 選んだAI社の公式ステータスを Gemini でにせい口調で説明する

`/nisei_summary` と `/nisei_ai_status` を使うには `.env` に `GEMINI_API_KEY` が必要です。モデルは `GEMINI_MODEL`（初期値 `gemini-3.5-flash`）、thinking は `GEMINI_THINKING_LEVEL`（初期値 `medium`）で変更できます。`/nisei_ai_status` の取得タイムアウトは `AI_STATUS_TIMEOUT_MS`（初期値 `8000`）です。直近24時間のテキストと画像添付を対象にし、長い日は分割要約してからマージします（待ち時間が増えることがあります）。最終まとめが長いときは「つづきみる？」ボタンで続きを同じメッセージ内に表示します。コマンド追加・変更後は `npm run deploy:commands` を実行してください。
