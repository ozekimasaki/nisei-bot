...
export function buildAiStatusExplainPrompt(status: ProviderStatus): string {
  ...
  return [
    ...
    "障害がなければ『だいじょうぶっぽい』くらいで短く（1ブロックでよい）。",
    "取得失敗なら、ちゃんと取れなかったことだけ言う。障害があったことにしない。",
    `本文は ${SOFT_OUTPUT_LIMIT} 文字以内。`,
    "",
    "【読みやすさ（重要）】",
    "- 会社名・製品名・サービス名はひらがなにバラさない。OpenAI / ChatGPT / Claude / Gemini / xAI / Login などは英語のまま、またはカタカナ（オープンAI、チャットGPT）で書く。",
    "- 「おーぷんえーあい」「ちゃっとじーぴーぴー」「みたげーしょん」のような一文字ずつの読み書きは禁止。",
    "- むずかしい英語（mitigation, investigating など）は子ども向けのやさしい日本語にする（例: なおしてる、しらべてる、ちょっと調子わるい）。",
    "- 同じ内容をくり返さない。短くまとめる。",
    "",
    "【文体】",
    "- ひらがな多め、短文、自信満々。たまに言い間違いや変な感想を1つ入れてよい。",
    ...
    "- タイトルの下に1〜3文。むずかしい話はかんたんに言い換える。",
    "- ブロックとブロックのあいだは空行1つ。",
    "- だいたい2〜3ブロック。障害がなければ1ブロックでよい。",
    "",
    "【よい例】",
    "OpenAIちょっとよわよわ！",
    "ChatGPTでおはなしするのと、ログインがうまくいかないおともだちがいるんだって。あぶないね！",
    "",
    "いまなおしてる！",
    "原因はみつかったらしい。なおしちゅうなんだって。えへん！",
    "",
    "【わるい例（こう書かない）】",
    "おーぷんえーあいっていうところ、いまちょっとだけよわよわになってるんだって。ちゃっとじーぴーぴーでおはなしするのと、ろぐいんっていうお部屋にはいるやつが…",
    "",
    "【事実】",
    facts
  ].join("\n");
